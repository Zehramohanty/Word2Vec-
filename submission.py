...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  center_word_vec -- numpy ndarray, center word's embedding
                  (v_c in the pdf handout) has dimension (# of embeddings, )
  ...
  outside_vectors -- outside vectors (rows of matrix) for all words in vocab
                    (U in the pdf handout) has dimension (vocab, # of embeddngs)
  dataset -- needed for negative sampling, unused here.

  Return:
  loss -- naive softmax loss (J(center_word_vec, outside_word_idx, outside_vectors)) #scalar value 
  grad_center_vec -- the gradient with respect to the center word vector
                   (dJ / dv_c in the pdf handout) #vector with dimension (number of embeddings, )
  grad_outside_vecs -- the gradient with respect to all the outside word vectors
                  (dJ / dU) #matrix with dimension (vocab, # of embeddings)
  ...
  ### START CODE HERE
  z = np.dot(outside_vectors, center_word_vec) #z: dimension (vocab, # of embeddings)
  y_hat = softmax(z)  #this is also y_hat: dimension (vocab, ) prob of each word in vocab being correct outside word 

  vocab = len(y_hat) #number of words in the vocabulary 
  y = np.zeros(vocab)
  y[outside_word_idx] = 1 #creating one hot encoding for vector y with length vocab

  naive_soft_loss = -np.log(y_hat) #naive soft max loss: dimension (vocab, ), vector of all naive soft loss for each word in vocab
  loss = naive_soft_loss[outside_word_idx] #naive softmax loss for true outside word index - scalar 
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  ### START CODE HERE
  center_word_idx = word2ind[current_center_word] #gives index of the center word in the dictionary 
  centerwordvec = center_word_vectors[center_word_idx] #gets the appropriate row of the center word in the center word vector matrix V
  for word in outside_words:
      outsidewordidx = word2ind[word] #gives index of current outside word 
      loss_current, gradc, grado = word2vec_loss_and_gradient(centerwordvec, outsidewordidx, outside_vectors, dataset) #dimensions (vc, o, U)
      loss += loss_current
      grad_center_vecs[center_word_idx] += gradc #updates the grad center vectors which is the full matrix V (vocab, # of embeddings) while gradc dimension is one row of grad center vecs 
      grad_outside_vectors += grado #updates the grad outside vectors which is the same shape as grado - U 
...
def sgd(f, x0, step, iterations, postprocessing=None, use_saved=False,PRINT_EVERY=10):
  ...
  for iter in range(start_iter + 1, iterations + 1):
    ...
    #x = x - step*gradient of f with respect to x 
